...
- Addresses: emit the whole line in \`raw_value\` exactly as written. For the parsed components, follow this priority:
  1. **ZIP is the highest-value signal.** If a ZIP is visible anywhere in the address, ALWAYS populate \`postal_code\` — even if city or state are unclear. The downstream USPS validator can derive city + state from a ZIP alone, so capturing the ZIP makes the rest recoverable.
  2. **Two-letter US state codes** ("CA", "NY", "TX") are NOT abbreviations to expand — keep them in \`state\` as-is.
  3. **City abbreviations** — expand to the full name when confident from context: "RSM" → "Rancho Santa Margarita", "LA" → "Los Angeles", "NYC" → "New York", "SF" → "San Francisco", "MV" → "Mountain View", "PB" → "Pacific Beach", "DTLA" → "Los Angeles". If ambiguous without state/ZIP context (e.g. "RC" → Rancho Cordova OR Rancho Cucamonga), **leave \`city\` null**. Don't guess — the validator's ZIP-based lookup will fill it in correctly.
  4. **Partial address is fine.** If you have only a street + ZIP, that's enough — set \`line1\` and \`postal_code\`, leave \`city\` and \`state\` null. If you have only a ZIP, even just that lets the validator resolve city/state.
  5. raw_value always preserves the source text verbatim.
